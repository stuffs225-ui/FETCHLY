...
export function RequestForm() {
      ...
      <Card className="mx-auto max-w-lg p-8 text-center">
        ...
        <div className="mt-6 flex items-center justify-between gap-3 rounded-xl border border-border-light bg-surface p-4">
          <div className="text-start">
            ...
            <p className="mt-1 font-mono text-lg font-bold text-primary">{result.requestNumber}</p>
          </div>
          <button
            onClick={() => {
            ...
            className="flex h-9 w-9 items-center justify-center rounded-lg border border-border-light hover:bg-black/5"
          >
  ...
  return (
    <Card id="request-form-top" className="mx-auto max-w-2xl p-6 sm:p-8">
      <div className="space-y-5">
        ...
        <div>
          <FieldLabel>{t.requestForm.fields.productUrl} ({t.common.optional})</FieldLabel>
        ...
        <div>
          <FieldLabel>{t.requestForm.fields.description} ({t.common.optional})</FieldLabel>
        ...
        <div>
          <button type="button" onClick={() => setShowMore((v) => !v)} className="flex items-center gap-1.5 text-sm font-semibold text-primary">
            <ChevronDown className={cn('h-4 w-4 transition-transform', showMore && 'rotate-180')} />
            {t.requestForm.moreDetails}
          </button>

          <div className={cn('grid overflow-hidden transition-all duration-300', showMore ? 'mt-4 grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0')}>
                  ...
                  {(Object.keys(t.requestForm.sourceOptions) as SourcePreference[]).map((key) => (
                    <button
                      ...
                      className={cn(
                        ...
                        form.sourcePreference === key ? 'border-primary bg-primary/10 text-primary' : 'border-border-light text-text-muted hover:border-primary/40',
                      )}
                    >
        ...
        <div className="border-t border-border pt-6">
          <label className="flex cursor-pointer items-start gap-3">
            ...
            <span className="text-sm text-text-muted">
              ...
              <a href="/legal/privacy" className="font-semibold text-primary hover:underline">
